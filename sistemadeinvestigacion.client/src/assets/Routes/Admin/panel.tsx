...
/** * PANEL UNIFICADO V3.3 - PDI Intranet 2026
 * Fix: Proporción de imagen real (no zoom), bordes suavizados y visibilidad en zoom 100%.
 */
...
export default function Panel() {
    ...
    const quickTransition = { duration: 0.3, ease: [0.23, 1, 0.32, 1] };
    ...
    return (
        <div className="fixed inset-0 overflow-hidden bg-white font-sans text-slate-900 selection:bg-[#002855] selection:text-white">
            ...
            <div className="h-full overflow-y-auto snap-y snap-mandatory scroll-smooth overflow-x-hidden">
                ...
                {/* SECCIÓN 1: HERO */}
                <section className="snap-start w-full h-screen flex flex-col items-center justify-center px-6 relative bg-[#002855]">
                    ...
                    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto text-center z-10">
                        <h2 className="text-blue-400 font-bold uppercase tracking-[0.3em] mb-4 text-sm">Bienvenido, {userName}</h2>
                ...
                {/* SECCIÓN 2: CARRUSEL REVISADO */}
                <section ref={carouselRef} className="snap-start w-full h-screen flex flex-col justify-center bg-slate-50 relative overflow-hidden pt-12 md:pt-16">
                    <div className="max-w-7xl mx-auto w-full px-6 mb-2 text-center shrink-0">
                        <h2 className="text-3xl md:text-4xl font-black text-[#002855] tracking-tighter mb-2 uppercase">Acuerdos Destacados</h2>
                        <div className="w-20 h-1 bg-blue-600 mx-auto rounded-full" />
                    </div>

                    <div className="w-full relative px-4 py-8 overflow-visible">
                        <div className="embla overflow-visible" ref={emblaRef} key={mejores.length}>
                                ...
                                {(mejores.length > 0 ? mejores : acuerdos.slice(0, 6)).map((a, index) => {
                                    ...
                                    return (
                                        <div key={`destacado-${a.id}-${index}`} className="embla__slide flex-[0_0_90%] md:flex-[0_0_55%] lg:flex-[0_0_42%] px-3">
                                            ...
                                            >
                                                {/* Contenedor con Aspect Ratio 4:3 estricto para imagen 800x600 */}
                                                ...
                                                <div className="p-6 md:p-8 bg-white border-t border-slate-100 min-h-[140px] flex flex-col justify-center">
                                                    ...
                                                    <h3 className="text-xl md:text-2xl font-black text-[#002855] uppercase leading-tight mb-2 truncate">
                                                        {a.titulo}
                                                    </h3>
                                                    <p className="text-slate-500 text-xs md:text-sm font-medium italic line-clamp-2">
                                                        {a.descripcion}
                                                    </p>
                                                </div>
                                            </motion.div>
                    ...
                    <div className="flex justify-center gap-3 mt-2">
                        {scrollSnaps.map((_, i) => (
                            <button key={i} onClick={() => emblaApi?.scrollTo(i)} className={`h-1.5 transition-all rounded-full ${i === selectedIndex ? 'w-10 bg-[#002855]' : 'w-2 bg-slate-300'}`} />
                        ))}
                    </div>
                </section>
                ...
                <section ref={listSectionRef} className="snap-start w-full h-screen bg-[#002855] text-white py-12 px-6 flex items-center overflow-hidden">
                    <div className="max-w-6xl mx-auto w-full flex flex-col h-[85vh]">
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 shrink-0">
                            ...
                            <input
                                ...
                                className="bg-white/10 border border-white/10 text-white px-8 py-4 rounded-xl outline-none font-bold w-full md:w-80 text-base focus:bg-white focus:text-[#002855] transition-all"
                            />
                        </div>
                        <div className="flex-1 overflow-y-auto pr-4 space-y-3 custom-list-scroll pb-10">
                            {loading ? (
                                <div className="text-center py-20 animate-pulse text-blue-300 uppercase tracking-widest">Sincronizando...</div>
                            ) : filtered.map((a) => (
            ...
            {/* MODAL (Proporción exacta 4:3 para el div de imagen) */}
            <AnimatePresence>
                {modalData && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-5xl rounded-3xl overflow-hidden relative z-10 shadow-2xl flex flex-col md:grid md:grid-cols-2 md:grid-rows-2 h-[90vh] md:h-[75vh]">
                            ...
                            <div className="p-8 md:p-10 flex flex-col justify-center border-r border-slate-200 bg-white">
                                <h3 className="text-2xl md:text-3xl font-black text-[#002855] uppercase leading-none mb-3">{modalData.titulo}</h3>
                                <p className="text-base md:text-lg text-slate-500 font-medium italic">"{modalData.descripcion}"</p>
                            </div>
