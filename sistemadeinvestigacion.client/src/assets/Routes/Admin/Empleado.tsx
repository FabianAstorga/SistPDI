...
export default function AdministracionIdentidad() {
    ...
    // 1. Limpieza de Body y AbortController (Anti-Leak)
    useEffect(() => {
            ...
            <main className="relative z-10 flex-1 p-6 pt-28 mb-4 flex flex-col items-center overflow-hidden">
                <div className="w-full max-w-[1500px] flex flex-col h-full gap-4">
                    ...
                    <div className="flex-1 bg-white flex flex-col overflow-hidden rounded-sm shadow-2xl relative">
                        {/* 2. Línea divisoria en el fondo (Higiene visual) */}
...
const IdentityRow = memo(({ data }: { data: any }) => {
    ...
    // Formateo de fecha de creación del usuario
    const creationDate = useMemo(() => {
        if (!data.usuario?.fechaCreacion) return null;
        return new Date(data.usuario.fechaCreacion).toLocaleDateString();
    }, [data.usuario?.fechaCreacion]);

    return (
        <div className="px-6 py-1.5 bg-white">
            <div
                onClick={() => console.log("Gestionar:", data.funcionario.rut)}
            ...
            >
                ...
                <div className="flex items-center gap-5 p-5 pr-10">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                            ...
                            {isLinked && (
                                <span className="text-[8px] font-black px-1.5 py-0.5 rounded-sm bg-emerald-100 text-emerald-700 uppercase">
                                    Verificado
                                </span>
                            )}
                        </div>
                ...
                {/* SECCIÓN CREDENCIALES (Atributos extendidos) */}
                <div className={`flex items-center gap-5 p-5 pl-10 ${isLinked ? '' : 'border-l border-slate-200 opacity-60'}`}>
                    {isLinked ? (
                        <div className="flex-1 grid grid-cols-2 gap-4">
                            <div>
                                <p className="text-[10px] font-black text-[#002855] uppercase tracking-tighter leading-none mb-1">ID Persona: {data.usuario.idPersona}</p>
                            ...
                            <div className="flex flex-col justify-center border-l border-slate-200 pl-4">
                                <div className="flex items-center gap-2 text-slate-400">
                                    <Calendar size={11} />
                                    <span className="text-[9px] font-bold uppercase">Registrado: {creationDate}</span>
                                </div>
                                {/* Lógica de Roles: 1 Admin, 2 Funcionario */}
                                <span className={`text-[8px] font-black uppercase mt-1 ${data.usuario.rol === 1 ? 'text-blue-600' : 'text-slate-600'}`}>
                                    Privilegios: {data.usuario.rol === 1 ? 'Administrador ' : 'Funcionario '}
                                </span>
                            </div>
                        </div>
                    ) : (
                        <div className="flex-1 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Sin credenciales</p>
                                    <p className="text-[9px] text-slate-400 uppercase mt-1 italic">Vínculo pendiente</p>
                                </div>
                            </div>
