...
// 1. Memoizamos los mini iconos para que se rendericen UNA SOLA VEZ en toda la sesión
const FiguraMiniIcon = memo(({ id }: { id: string }) => {
    const common = { width: 22, height: 22, viewBox: '0 0 100 100', className: 'block' };
    const shapes: Record<string, React.ReactNode> = {
        rectangulo: <rect x="18" y="26" width="64" height="48" rx="6" fill="currentColor" opacity="0.85" />,
        circulo: <circle cx="50" cy="50" r="28" fill="currentColor" opacity="0.85" />,
        triangulo: <polygon points="50,18 18,82 82,82" fill="currentColor" opacity="0.85" />,
        rombo: <polygon points="50,16 84,50 50,84 16,50" fill="currentColor" opacity="0.85" />,
        hexagono: <polygon points="30,18 70,18 88,50 70,82 30,82 12,50" fill="currentColor" opacity="0.85" />,
        octagono: <polygon points="32,14 68,14 86,32 86,68 68,86 32,86 14,68 14,32" fill="currentColor" opacity="0.85" />,
        estrella: <polygon points="50,10 60,38 90,38 66,56 76,86 50,68 24,86 34,56 10,38 40,38" fill="currentColor" opacity="0.85" />
    };
    return <svg {...common}>{shapes[id] || <circle cx="50" cy="50" r="8" fill="currentColor" />}</svg>;
});

const TrazoMiniIcon = memo(({ id }: { id: 'linea' | 'flecha' | 'curva' }) => {
    const common = { width: 22, height: 22, viewBox: '0 0 100 100', className: 'block' };
    if (id === 'linea') return (
        <svg {...common}><line x1="18" y1="78" x2="82" y2="22" stroke="currentColor" strokeWidth="10" strokeLinecap="round" /></svg>
    );
    if (id === 'flecha') return (
        <svg {...common}>
    ...
    return (
        <svg {...common}>
            <path d="M 18 78 Q 50 18 82 42" fill="none" stroke="currentColor" strokeWidth="10" strokeLinecap="round" />
            <circle cx="18" cy="78" r="6" fill="currentColor" /><circle cx="82" cy="42" r="6" fill="currentColor" />
        </svg>
    );
});
...
export const LeftToolbar: React.FC<Props> = memo(({ model }) => {
    ...
    return (
        <aside className="w-24 bg-white border-r border-gray-200 flex flex-col items-center py-4 space-y-4 shadow-sm z-20">
            {/* Multi selección */}
            <button
                onClick={() => activarHerramienta('multiseleccion')}
            ...
            {/* Goma */}
            <button
                onClick={() => activarHerramienta('goma')}
            ...
            <div className="relative">
                <button
                    ...
                    className={sidebarBtnClass(FIGURAS?.some((x: any) => x.id === herramientaActiva))}
                    title="Figuras"
                >
                    <Shapes size={22} />
                </button>
                    ...
                    <div className="absolute left-16 top-0 bg-white shadow-2xl border border-gray-200 rounded-xl p-2 grid grid-cols-3 gap-2 z-50 w-56 animate-in fade-in slide-in-from-left-2">
                        {FIGURAS?.map((fig: any) => (
                            <button
                                ...
                                className="p-2 hover:bg-blue-50 rounded-lg text-gray-600 flex flex-col items-center justify-center transition-colors group"
                                title={fig.label}
                            >
                    ...
                    <div className="absolute left-16 top-0 bg-white shadow-2xl border border-gray-200 rounded-xl p-2 grid grid-cols-1 gap-2 z-50 w-44 animate-in fade-in slide-in-from-left-2">
                        {TRZ.map((t) => (
                            <button
                                ...
                                className="p-2 hover:bg-blue-50 rounded-lg text-gray-600 flex items-center gap-2 transition-colors group"
                                title={t.label}
                            >
                                <div className="text-gray-700 group-hover:text-blue-600">
            ...
            {/* Lápiz */}
            <button
                onClick={() => activarHerramienta('lapiz')}
            ...
            {/* Texto */}
            <button
                onClick={() => activarHerramienta('texto')}
