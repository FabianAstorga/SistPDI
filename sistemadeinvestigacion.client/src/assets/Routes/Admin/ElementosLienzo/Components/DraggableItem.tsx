    ...
    return (
        ...
        >
            {estaSeleccionado && el.type !== 'lapiz' && (
                <g data-editor="1">
                    <rect x={-6} y={-6} width={w + 12} height={h + 12} fill="none" stroke="#3b82f6" strokeWidth="2" strokeDasharray="4" pointerEvents="none" />
                    {['nw', 'ne', 'sw', 'se'].map((corner) => (
                        <circle key={corner} cx={corner.includes('e') ? w : 0} cy={corner.includes('s') ? h : 0} r={10} fill="white" stroke="#3b82f6" strokeWidth="2" className={`cursor-${corner}-resize`} onMouseDown={(e) => iniciarResize(e, corner)} />
                    ))}
                </g>
            )}
